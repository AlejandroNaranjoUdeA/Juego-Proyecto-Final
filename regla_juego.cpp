...
void regla_juego::generate_map() {
    ...
    // Las tres primeras filas no tienen nada excepto el fondo
    for (unsigned int fil = 3; fil < game_map_rows - 1; fil++) {
        if (fil == 3) {
            setup_rocas(fil, 4); // Columna 4
        } else if (fil == 4) {
            setup_enemigos(fil, 5); // Columna 5
        } else if (fil == 6) {
            ...
            if ((fil - 10) % 2 == 0) {
                setup_enemigos(fil, 13); // Columna 13
            }
        }
    }
}
...
bool regla_juego::check_collision_with_roca(QGraphicsPixmapItem *item) {
    for (auto roca : rocars) { // Iterar sobre todos los enemigos
